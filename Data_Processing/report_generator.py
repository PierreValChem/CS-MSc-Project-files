"""
Report generation module for NMReDATA converter
"""
...
class ReportGenerator:
    ...
    def generate_report(self):
        ...
        total_original = total + self.metrics['compounds_without_nmr']
        ...
        accuracy_score = (
            ...
            txt_found_pct * 0.15 +  # 15% weight for finding files
            valid_smiles_pct * 0.2 +  # 20% weight for valid SMILES
            gen_3d_pct * 0.25 +  # 25% weight for 3D generation (increased importance)
        ...
        report = self._format_report(
            total, total_original, nmr_coverage_pct, txt_found_pct,
            valid_smiles_pct, gen_3d_pct, peaks_parsed_pct, mapping_pct,
            overall_success_pct, accuracy_score
        )
    ...
    def _format_report(self, total, total_original, nmr_coverage_pct, txt_found_pct,
                      valid_smiles_pct, gen_3d_pct, peaks_parsed_pct, mapping_pct,
                      overall_success_pct, accuracy_score):
...
Structure Processing:
  ...
  - Empty MOL blocks:          {self.metrics['empty_mol_blocks']}
...
System Issues:
  - Memory issues:             {self.metrics['memory_issues']}
  - Timeout issues:            {self.metrics['timeout_issues']}
...
-------------"""
        ...
        if self.metrics['compounds_without_nmr'] > 0:
            issues.append(f"- Missing NMR data: {self.metrics['compounds_without_nmr']} compounds had no NMR data files (pre-filtered)")
        ...
        if self.metrics['empty_mol_blocks'] > 0:
            issues.append(f"- Empty MOL blocks: {self.metrics['empty_mol_blocks']} files were not created due to invalid structure data")
        ...
        if self.metrics['consolidation_warnings'] > 0:
            issues.append(f"- Peak consolidation warnings: {self.metrics['consolidation_warnings']} files may have incorrect peak grouping")
